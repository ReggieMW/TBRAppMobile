...
namespace TBRAppMobile.Helpers
{
    public static class AnimationExtensions
    {
        public static Task<bool> ColorTo(this VisualElement self,
            Color fromColor, Color toColor, Action<Color> callback,
            uint length = 250, Easing? easing = null)
        {
            ...
            return self.AnimateAsync("ColorTo", anim, 16, length, easing ?? Easing.Linear);
        }

        public static Task<bool> AnimateAsync(this VisualElement element,
            string name, Animation animation, uint rate, uint length, Easing easing)
        {
            var tcs = new TaskCompletionSource<bool>();
            animation.Commit(element, name, rate, length, easing,
                (v, cancelled) => tcs.SetResult(!cancelled));
            return tcs.Task;
        }
    }
}
